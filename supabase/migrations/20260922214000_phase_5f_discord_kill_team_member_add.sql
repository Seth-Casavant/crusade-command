...
create or replace function public.discord_add_campaign_kill_team_member(
  p_campaign_id uuid,
...
begin
  ...
  if length(btrim(coalesce(p_member_display_name, ''))) = 0 then
    raise exception using
      errcode = 'P0001',
      message = 'KILL_TEAM_MEMBER_DISPLAY_NAME_REQUIRED';
  end if;
  ...
  where team.campaign_id = p_campaign_id
    and team.leader_discord_user_id = btrim(p_leader_discord_user_id)
  for update;
  ...
  if exists (
    ...
    where member.campaign_id = p_campaign_id
      and member.discord_user_id = btrim(p_member_discord_user_id)
  ) then
    raise exception using
      errcode = 'P0001',
  ...
  loop
    ...
    v_processed_mission_count := v_processed_mission_count + 1;
  end loop;
...
revoke all on function public.discord_add_campaign_kill_team_member(
  uuid,
  text,
  text,
  text
...
grant execute on function public.discord_add_campaign_kill_team_member(
  uuid,
  text,
  text,
  text
...
comment on function public.discord_add_campaign_kill_team_member(
  uuid,
  text,
  text,
  text
) is
  'Service-role-only Discord roster path. Allows a Kill Team Leader to add one Discord member to the campaign roster and all linked mission rosters before scoring begins.';
